...
Wait-ForLogText -InstallRoot $installRoot -Text 'Installed Osiris Beta_1.0.32'

$installedVersion = (Get-Content -LiteralPath (Join-Path $installRoot 'version.json') -Raw | ConvertFrom-Json).version
if ($installedVersion -ne 'Beta_1.0.32') { throw "Unexpected installed version: $installedVersion" }
...
$failureApi = New-TestRelease -Version 'Beta_1.0.33' -Marker 'must-rollback' -ForceApplyFailure
...
if ($rolledBackVersion -ne 'Beta_1.0.32') { throw "Rollback left version $rolledBackVersion" }
if ((Get-Content -LiteralPath (Join-Path $installRoot 'App\release-marker.txt') -Raw).Trim() -ne 'updated') {
...
Write-Output '  Beta_1.0.31 -> Beta_1.0.32 installed'
Write-Output '  Data sentinel preserved'
Write-Output '  forced Beta_1.0.33 failure rolled back to Beta_1.0.32'
